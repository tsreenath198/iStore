...
public interface OrderDetailRepository extends CrudRepository<OrderDetail, Integer> {
    ...
    @Query(value = "select od.id as id,year(od.invoice_date) as year,month(od.invoice_date) as month,'year' as type, year(od.invoice_date) as value, "
            + " ROUND(SUM(CASE WHEN od.payment_mode='Cash' THEN od.total ELSE 0 END)) as cashSum ,ROUND(SUM(CASE WHEN od.payment_mode='Bank' "
            + "THEN od.total ELSE 0 END)) as bankSum , ROUND(SUM(od.total)) as sum from order_detail od where od.invoice_date > :fromDate and od.invoice_date <:toDate"
            + " and od.active_flag=0 group by Year(od.invoice_date) ORDER BY od.invoice_date DESC", nativeQuery = true)
    List<OrderDetailGroupInterface> findAllByGroupYears(@Param(value = "fromDate") String fromDate,
                                                        @Param(value = "toDate") String toDate);

    @Query(value = "select od.id as id,year(od.invoice_date) as year,month(od.invoice_date) as month,'month' as type, Month(od.invoice_date) as value,"
            + " ROUND(SUM(CASE WHEN od.payment_mode='Cash' THEN od.total ELSE 0 END)) as cashSum ,ROUND(SUM(CASE WHEN od.payment_mode='Bank' "
            + "THEN od.total ELSE 0 END)) as bankSum , ROUND(SUM(od.total)) as sum from order_detail od where od.invoice_date > :fromDate and "
            + "od.invoice_date <:toDate and od.active_flag=0 group by Month(od.invoice_date) ORDER BY od.invoice_date DESC", nativeQuery = true)
    List<OrderDetailGroupInterface> findAllByGroupMonths(@Param(value = "fromDate") String fromDate,
                                                         @Param(value = "toDate") String toDate);

    @Query(value = "select od.id as id,year(od.invoice_date) as year,month(od.invoice_date) as month,'day' as type, Day(od.invoice_date) as value, ROUND(SUM(CASE WHEN od.payment_mode='Cash' THEN od.total ELSE 0 END)) as cashSum ,ROUND(SUM(CASE WHEN od.payment_mode='Bank' THEN od.total ELSE 0 END)) as bankSum , ROUND(SUM(od.total)) as sum from order_detail od where od.invoice_date > :fromDate and od.invoice_date < :toDate and od.active_flag=0 group by Year(od.invoice_date), Month(od.invoice_date) , Day(od.invoice_date) ORDER BY invoice_date DESC", nativeQuery = true)
    ...
    @Query(value = "select od.id as id,year(od.created_date) as year,month(od.created_date) as month,'month' as type, Month(od.created_date) as value, "
            ...
            + " group by Month(od.created_date) ORDER BY invoice_date DESC", nativeQuery = true)
    ...
    @Query(value = "select * from order_detail od where year(od.invoice_date)=:year and month(od.invoice_date)=:month and day(od.invoice_date)=:value and od.active_flag=0 ", nativeQuery = true)
